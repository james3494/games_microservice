...
module.exports = ({ tests, method, editEntity, entity }) => {
  tests.forEach((test) => {
    ...
    it(should, (done) => {
      ...
      request(app)
        ...
        .query(test.query)
        .set(
          test.apiKeyOverride === null ? "X-Other" : "X-Api-Key",
          test.apiKeyOverride || process.env.API_KEY
        )
        ...
        .end((error, res) => {
          try {
            ...
            if (typeof test.expectedBody === "function") {
              ...
              Object.entries(test.expectedBody || {}).forEach(
                ([key, value]) => {
                  if (value === null || value === "null")
                    expect(res.body[key]).to.be.null;
                  else if (value === "notnull")
                    expect(res.body[key]).not.to.be.null;
                  else if (typeof value == "object")
                    expect(JSON.stringify(res.body[key])).to.be.equal(
