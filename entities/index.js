...
const makeTakeout = buildMakeTakeout({
    Id,
    throwError,
    validate: buildValidate("takeout", buildTakeoutValidation({
        Id 
    }))
});
...
const makeGame = buildMakeGame({
    Id,
    throwError,
    validate: buildValidate("game", buildGameValidation({
        Id 
    }))
});
const makeTakeoutMethod = buildMakeTakeoutMethod({
    Id,
    validate: buildValidate("takeoutMethod", buildTakeoutMethodValidation({
        Id 
    }))
});
...
const makeRating = buildMakeRating({
    Id,
    validate: buildValidate("rating", buildRatingValidation({
        Id 
    }))
});
...
const makePack = buildMakePack({
    Id,
    validate: buildValidate("pack", buildPackValidation({
        Id 
    }))
});
...
const makePackPurchase = buildMakePackPurchase({
    Id,
    validate: buildValidate("packPurchase", buildPackPurchaseValidation({
        Id 
    }))
});
